...
for x,model in enumerate(tqdm(models[0:])):
    ...
    time_total = invert_times[]  
    
    nsteps = int(time_total/tstep_interval)
    
    # Get inversion timesteps in equal increments of strain
    start = times
    start_step = int(start/tstep_interval)
    ...
    times = timesteps*tstep_interval
    ...
    for k,step in enumerate(tqdm(timesteps)):
        ...
        y_values = strains_summed_clipped['Strain_Rate']
    ...
    fig.savefig(str(k+1)+'_strain_rate_results.jpg')
